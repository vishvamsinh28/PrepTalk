/**
 * @file Response helpers giving every API route one JSON shape:
 * `{ success: false, error, message }` on failure. `error` is the field to
 * read; `message` duplicates it for older client code.
 */
 ...
 * The message is shown to the user, so say what to correct rather than naming
 * internals.
 *
 ...
 * Keep the message generic — stack traces and driver errors belong in
 * `console.error`, not in a response body.
 *
