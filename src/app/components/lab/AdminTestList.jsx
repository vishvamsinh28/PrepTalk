      ...
      <section className="glass-panel rounded-xl p-4 sm:p-5">
        <div>
          ...
          <div className="mt-4 flex flex-wrap items-end justify-between gap-4">
            <div>
              <h1 className="text-3xl font-semibold tracking-tight gradient-text sm:text-4xl">Lab assessments</h1>
            ...
            <div className="grid w-full grid-cols-2 gap-3 sm:flex sm:w-auto">
              <button onClick={onExport} className="inline-flex items-center justify-center gap-2 rounded-lg border border-rule bg-black/5 px-4 py-3 font-bold text-ink">
                <FaDownload />
                Export
              </button>
              <button onClick={onCreate} className="inline-flex items-center justify-center gap-2 rounded-lg bg-ink px-4 py-3 font-semibold text-canvas sm:px-5">
                <FaPlus />
                Create assessment
              </button>
            </div>
      ...
      <section className="mt-4 grid min-h-0 flex-1 gap-4 lg:grid-cols-[20rem_minmax(0,1fr)]">
        <aside className="glass-panel rounded-xl p-4 lg:self-stretch">
          ...
          <label className="block">
            ...
            <span className="flex items-center gap-3 rounded-lg border border-rule bg-white px-4 py-3">
          ...
          <div className="mt-4 rounded-lg border border-accent bg-accent/10 p-4 text-sm leading-6 text-ink-soft">
            Showing {assessments.length} matching assessment{assessments.length === 1 ? "" : "s"}.
          </div>
        </aside>

        <section className="flex min-h-[18rem] flex-col overflow-hidden rounded-xl border border-rule bg-white shadow-xl lg:min-h-0">
  ...
  return (
    <article className="grid gap-4 px-4 py-5 text-ink md:grid-cols-[minmax(0,1fr)_6.5rem_6.5rem_5rem] md:items-center md:px-5 md:py-4">
      ...
      <div className="grid grid-cols-3 gap-3 md:contents">
        <span className="rounded-lg border border-rule bg-black/5 px-3 py-2 text-sm md:border-0 md:bg-transparent md:px-0 md:py-0 md:text-center md:text-base">
        ...
        <button onClick={onDelete} className="rounded-lg border border-rose-200/20 bg-rose-200/5 px-3 py-2 text-left text-sm text-rose-700 md:border-0 md:bg-transparent md:px-0 md:py-0 md:text-center md:text-base" title="Delete assessment" aria-label={`Delete ${assessment.title}`}>
